...
object KernelBoiler {
  ...
  def main(args: Array[String]): Unit = {
    gen(Paths.get(System.getenv("SHARED")).resolve("cats/kernel").toFile)
  }
  ...
  object GenTupleInstances extends Template {
    ...
    def filename(root: File): File = root.toPath.resolve("cats").resolve("kernel").resolve("instances").resolve("TupleInstances.scala").toFile
