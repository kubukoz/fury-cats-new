...
object Boilerplate {
  ...
  def main(args: Array[String]): Unit = {
    gen(Paths.get(System.getenv("SHARED")).resolve("cats/cats").toFile)
  }
  ...
  def gen(dir: File) = for (template <- templates) yield {
    val tgtFile = template.filename(dir)
    tgtFile.getParentFile.mkdirs()
    Files.write(tgtFile.toPath, template.body.getBytes(), StandardOpenOption.CREATE)
    tgtFile
  }
  ...
  object GenSemigroupalBuilders extends Template {
    def filename(root: File): File = (root.toPath.resolve("cats").resolve("syntax").resolve("SemigroupalBuilder.scala")).toFile
  ...
  object GenApplyArityFunctions extends Template {
    def filename(root: File) = (root.toPath.resolve("cats").resolve("ApplyArityFunctions.scala")).toFile
  ...
  object GenParallelArityFunctions extends Template {
    def filename(root: File) = (root.toPath.resolve("cats").resolve("ParallelArityFunctions.scala")).toFile
  ...
  object GenParallelArityFunctions2 extends Template {
    def filename(root: File) = root.toPath.resolve("cats").resolve("ParallelArityFunctions2.scala").toFile
    override def range = 2 to maxArity
    def content(tv: TemplateVals) = {
      import tv._
  ...
  object GenSemigroupalArityFunctions extends Template {
    def filename(root: File) = root.toPath.resolve("cats").resolve("SemigroupalArityFunctions.scala").toFile
    override def range = 2 to maxArity
    def content(tv: TemplateVals) = {
      import tv._
  ...
  object GenTupleParallelSyntax extends Template {
    def filename(root: File) = root.toPath.resolve("cats").resolve("syntax").resolve("TupleParallelSyntax.scala").toFile
  ...
  object GenTupleSemigroupalSyntax extends Template {
    def filename(root: File) = root.toPath.resolve("cats").resolve("syntax").resolve("TupleSemigroupalSyntax.scala").toFile
